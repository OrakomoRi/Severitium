...
class EnhancedBackgroundAnimation {
	...
	createParticles() {
		...
		this.particles = initialPositions.map((pos, index) => new EnhancedParticle(
			pos.x, 
			pos.y, 
			index + 1,
			this.options
...
function initializeBackgroundAnimation() {
	try {
		backgroundAnimation = new EnhancedBackgroundAnimation({
			// Constant speed smooth movement settings
			constantSpeed: 0.15,
...
window.BackgroundAnimation = {
	instance: () => backgroundAnimation,
	setSpeed: (speed) => backgroundAnimation?.setSpeed(speed),
	setDirectionChangeRate: (rate) => backgroundAnimation?.setDirectionChangeRate(rate),
	start: () => backgroundAnimation?.start(),
	stop: () => backgroundAnimation?.stop(),
	destroy: () => backgroundAnimation?.destroy()
};